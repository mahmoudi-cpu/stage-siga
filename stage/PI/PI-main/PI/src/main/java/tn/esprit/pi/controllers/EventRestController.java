...
public class EventRestController {
 ...
 /* @PostMapping("/assignshrtoevent/{eventName}")
    ...
    public ResponseEntity<String> assignshrtoevent(@RequestBody ShareHolder shareHolder, @PathVariable("eventName") String eventName) {
        ...
        if (eventId != null) {
            ...
            eventService.assignshrtoevent(eventId, shareHolder);

            // Vérifiez si l'événement est déjà complet avant d'investir
            if (event.getEventStatus() != EventStatus.COMPLETED) {
                try {
                    // Investir dans l'événement
                    shareHolderService.investInEvent(shareHolder.getIdShareholder(), eventId, event.getInvestNeeded());
    ...
    @PostMapping("/assignshrtoevent/{idEvent}")
    @PreAuthorize("hasAnyAuthority('ADMIN', 'SHAREHOLDER')")
    public ResponseEntity<String> assignshrtoevent(@RequestBody ShareHolder shareHolder, @PathVariable("idEvent") Integer idEvent) {


        // Affecter l'événement au shareholder en utilisant l'ID récupéré
        eventService.assignshrtoevent(idEvent, shareHolder);
    ...
    public ResponseEntity<Void> likeEvent(@PathVariable String eventName) {
        ...
        // Retourner une réponse HTTP vide avec un code de statut 204 "No Content"
        return ResponseEntity.noContent().build();
    }

    @PostMapping("/events/{eventName}/dislike")
    @PreAuthorize("hasAnyAuthority('SHAREHOLDER','CUSTOMER','AGENT','INVESTOR')")
    public void dislikeEvent(@PathVariable String eventName) {
    ...
    public ResponseEntity<String> cancelEvent(@PathVariable int eventId) {
        ...
        if (optionalEvent.isPresent()) {
            ...
            // Charger explicitement la collection de ShareHolders avant que la session Hibernate ne soit fermée
            Hibernate.initialize(event.getShareHolders());
    ...
    public ResponseEntity<List<Event>> historiqueEvent(@PathVariable("iduser") int userid) {
        ...
        String Text1 = "l'historique des offres de l'utilisateur " + user.getFirstname()+ "-" + user.getLastname();
