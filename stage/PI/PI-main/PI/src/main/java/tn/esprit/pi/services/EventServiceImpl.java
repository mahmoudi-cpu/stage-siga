...
public class EventServiceImpl implements EventService {
   ...
   EmailSenderService emailSenderService;
    ...
    public Event AddEvent(Event event, User authentication) {
        ...
        if (!userRole.equals(Role.ADMIN) && !userRole.equals(Role.SHAREHOLDER)) {
    ...
    public void addEvent(Event event, User authentication) {
        ...
        public void assignShareholderToEvent(int idShareholder, int idEvent) {
            // Récupérer l'actionnaire et l'événement correspondants depuis la base de données
            ShareHolder shareholder = iShareholderRepository.findById(idShareholder)
                    .orElseThrow(() -> new IllegalArgumentException("Actionnaire non trouvé avec l'ID: " + idShareholder));

            Event event = Ieventrepository.findById(idEvent)
                    .orElseThrow(() -> new IllegalArgumentException("Événement non trouvé avec l'ID: " + idEvent));

            // Assigner l'actionnaire à l'événement
            List<ShareHolder> shareholdersList = event.getShareHolders();
            shareholdersList.add(shareholder);
            event.setShareHolders(shareholdersList);
    ...
    @Override
    public void assignshrtoevent(Integer EventId, ShareHolder shareHolder) {
